...
const workspaceLabels: Record<Exclude<WorkspaceId, null>, string> = {
  video: 'Öppna video',
  dashboard: 'Öppna dashboard',
  tracking: 'Öppna spårning',
  agents: 'Öppna agenter',
  onboarding: 'Öppna onboarding',
  alarms: 'Skapa larm',
  forensic: 'Öppna forensic',
  map: 'Öppna karta',
...
function statusLabel(status: 'checking' | 'online' | 'offline', model: string, modelOk: boolean) {
  if (status === 'checking') return 'Ansluter till Ollama…'
  if (status === 'offline') return 'Ollama offline — kör ollama serve'
  if (!modelOk) return `${model} saknas — ollama pull ${model}`
  return `${model} · Ollama`
}
  ...
  const welcome: ChatMessage = {
    ...
    content:
      status === 'online' && modelAvailable
        ? `Hej! Jag kör lokalt via Ollama (${model}). Be mig visa video, onboarda kameror, skapa larm eller öppna dashboard.`
        : 'Hej! Jag är Smart VMS-copiloten. Starta Ollama med din Qwen-modell för full AI, annars används enkel intent-matchning.',
  }
    ...
    (trimmed: string): ChatMessage => {
      ...
      if (action) {
        ...
        return {
          id: id(),
          role: 'assistant',
          content: `${action.reply}\n\n_(Ollama ej tillgänglig — enkel intent-matchning.)_`,
          createdAt: new Date().toISOString(),
      ...
      return {
        ...
        role: 'assistant',
        content:
          'Ollama svarade inte. Kontrollera att `ollama serve` körs och att modellen finns (`ollama list`). Du kan prova snabbknapparna nedan.',
        createdAt: new Date().toISOString(),
      }
    },
    [openWorkspace],
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        let finalContent = content || 'Klart.'
        ...
        const hint =
          err instanceof OllamaError
            ? err.message
            : 'Kunde inte nå Ollama.'
        setMessages((m) => [
          ...m,
          {
            id: id(),
            role: 'assistant',
            content: `${hint}\n\nFörsöker enkel matchning istället.`,
            createdAt: new Date().toISOString(),
          },
      ...
      <div className="border-t border-slate-800/80 px-4 py-3">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            disabled={loading}
            placeholder="Fråga Copilot (Qwen via Ollama)…"
