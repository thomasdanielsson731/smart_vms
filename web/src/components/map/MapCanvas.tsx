...
export function MapCanvas({
  ...
  useEffect(() => {
    ...
    for (const cam of cameras) {
      ...
      wedge.bindTooltip(`${cam.name}: ${p.viewLabel || 'Vy'}`, { sticky: true })
      wedge.addTo(group)
  ...
  useEffect(() => {
    ...
    for (const pin of alarmPins) {
      ...
      const time = new Date(incident.occurredAt).toLocaleString('sv-SE')
      marker.bindPopup(
  ...
  useEffect(() => {
    ...
    L.marker([userLocation.lat, userLocation.lng], { icon, zIndexOffset: 1000 })
      .bindTooltip('Min plats', { direction: 'top', permanent: false })
      .addTo(group)
  }, [userLocation])
